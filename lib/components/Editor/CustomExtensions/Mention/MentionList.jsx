...
export class MentionList extends React.Component {
  ...
  render() {
    ...
    if (items.length === 0) {
      return (
        <div className="caliber-editor-mentions__wrapper">
          <p className="caliber-editor-mentions__item">No Results</p>
        </div>
      );
    }
    ...
    return (
      <div
        ref={this.mentionRef}
        className="caliber-editor-mentions__wrapper"
        data-cy="caliber-editor-mention-list"
      >
        {items.map(({ key, name, imageUrl, showImage }, index) => (
          <button
            className={classNames("caliber-editor-mentions__item", {
              active: index === selectedIndex,
            })}
            key={key}
            onClick={() => this.selectItem(index)}
            type="button"
            data-cy={`caliber-editor-mention-list-${name}`}
          >
